...
export default class ScratchpadRenderer extends React.Component {
  ...
  effectiveScale = () => (this.props.resolution || 1) * this.state.displayScale;
  ...
  componentDidMount = () => {
    ...
    this.redrawCanvas(context, this.effectiveScale());
  };
  ...
  componentDidUpdate = (prevProps: Props, prevState: State) => {
    ...
    if (
      ...
      if (this.props.strokes) {
        this.redrawCanvas(this.context, this.effectiveScale());
      }
  ...
  drawStroke = (
    ...
    if (
      ...
      initialOffset = Math.max(0, incrementalBaseStroke.samples.length - 1);
    }
  ...
  render = () => (
    <canvas
      ref="canvas"
      width={this.props.width * this.effectiveScale()}
      height={this.props.height * this.effectiveScale()}
      style={{
