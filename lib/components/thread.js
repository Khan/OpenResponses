...
type LockedThreadProps = {
  threadNumber: number,
};

export const LockedThread = ({ threadNumber }: LockedThreadProps) => {
  return (
    <div className={css(styles.container, styles.collapsedContainer)}>
      <div className={css(styles.postContainer)}>
        <div className={css(styles.lock)}>
          <img src="/static/lock.png" style={{ width: "100%" }} />
        </div>
        <div className={css(styles.contents)} style={{ alignSelf: "center" }}>
          <p className={css(styles.studentName)}>Partner #{threadNumber}</p>
        </div>
      </div>
    </div>
  );
