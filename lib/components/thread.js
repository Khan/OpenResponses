...
type PostProps = PostDisplayData & {
  ...
  shouldPromptForConfidence?: boolean,
...
const countReactions = (reactions, reactionType) =>
  (reactions &&
    Object.values(reactions).filter(r => r === reactionType).length) ||
  0;

class Post extends React.Component<PostProps, {}> {
  isPendingFocus = false;
  startedEditable = false;
  editorRef = null;
  ...
  componentDidMount = () => {
    ...
    if (this.props.isEditable) {
      if (this.props.shouldAutofocus && this.editorRef) {
  ...
  shouldComponentUpdate = (nextProps: PostProps, nextState) =>
    ...
    this.props.isStarActive !== nextProps.isStarActive ||
    this.props.isDownvoteActive !== nextProps.isDownvoteActive ||
  ...
  render = () => {
    ...
    const submitButtonTitle = validateSubmission(data)
      ? buttonTitle
      : "Write a little more?";
    ...
    return (
      <div className={css(styles.postContainer)}>
        ...
        {typeof confidence === "number" && (
          <div className={css(styles.confidenceBadge)}>
            <img
              src={`/static/confidence-icons/${confidence}.png`}
              style={{ width: "100%" }}
            />
          </div>
        )}
              ...
              {this.startedEditable && (
                <Fragment>
                  ...
                  <div className={css(styles.footerButtonContainer)}>
                    ...
                    {shouldPromptForConfidence ? (
                      <ConfidencePicker onSelectConfidence={onSubmit}>
                        <Button
              ...
              {reactionDisplayStyle !== "excluded" && (
                ...
                >
                  <button
                    ...
                    style={{
                      pointerEvents:
                        reactionDisplayStyle === "report" ? "none" : "unset",
                    }}
                  >
                    <img
                      src={
    ...
    if (this.props.pendingRichEditorData) {
      pendingPost = (
        <Post
          ...
          shouldPromptForConfidence={this.props.shouldPromptForConfidence}
        />
      );
