...
const normalizeStrokesForPlayback = strokes => {
  if (strokes.length > 0 && strokes[0].samples.length > 0) {
    ...
    return strokes.reduce((accumulatedStrokes, stroke, strokeIndex) => {
      const lastStrokeOutputEndTime =
        accumulatedStrokes.length > 0
  ...
  render = () =>
    <div
      ...
      style={{
        ...
        paddingBottom: this.props.paddingBottom || canvasPadding,
        backgroundColor: this.props.backgroundColor || "unset",
      }}
    >
      <Measure
