...
const validateData = data => {
  if (typeof data === "undefined" || data === null) {
    ...
    return Plain.serialize(State.fromJSON(JSON.parse(data.rawData))).length > 0;
      ...
      dataMappedChildren
        .map(
          child =>
            ...
            (!child.props ||
              !child.props.dataKey ||
              (child.type && child.type.skipsValidation) ||
              validateData(
        ...
        <div className={css(styles.bottomBar)}>
          <div className={css(styles.rightSide)}>
            <div className={css(styles.rightSideInterior)}>
              ...
              {!this.props.hasConnectivity ? (
                <p className={css(styles.connectivityWarning)}>
                  Having trouble reaching the internet! Trying to reconnect...
                </p>
              ) : null}
            </div>
          </div>
