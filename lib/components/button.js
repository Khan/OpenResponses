...
class Button extends React.Component {
  static propTypes = {
    ...
    // Specifies the hover/press style for the button.  Please use the
  ...
  state = {
    ...
    // This flag represents both press state on mobile and hover state on
    // desktop.
    pressed: false,
  };
  ...
  render() {
    ...
    const styleOverrides = pressed ? pressStyle || style : style;
    ...
    if (disabled) {
      ...
      rules.push(styles.press[type].kaGreen);
    } else if (!pressed) {
      rules.push(styles[type].kaGreen);
    }
...
styles.PRIMARY = StyleSheet.create({
  disabled: createSolidStyle(colors.gray76),
  kaGreen: createSolidStyle(colors.kaGreen),
});

styles.SECONDARY = StyleSheet.create({
  disabled: createOutlineStyle(colors.gray76),
  kaGreen: createOutlineStyle(colors.kaGreen),
});

styles.press = {};

styles.press.PRIMARY = StyleSheet.create({
  kaGreen: createSolidStyle(colors.kaGreenHover, createFocus(colors.kaGreen)),
});

styles.press.SECONDARY = StyleSheet.create({
  kaGreen: createOutlineStyle(colors.kaGreenHover, createFocus(colors.kaGreen)),
});
