...
const { colors, wbColors, wbTypography } = globalStyles;
...
class Button extends React.Component {
  ...
  render() {
    ...
    const { hideFocus, pressed, hover } = this.state;
    ...
    if (href) {
      ...
      return (
        <a href={disabled ? undefined : href} {...otherProps} {...commonProps}>
          {this.props.children}
        </a>
      );
    } else {
      ...
      return (
        <button disabled={disabled} {...otherProps} {...commonProps}>
          {this.props.children}
        </button>
      );
    }
...
const styles = StyleSheet.create({
  common: {
    ...wbTypography.labelLarge,
    padding: "0px 16px", // 18 + 16 + 16 = 40 (height)
    height: 40,
