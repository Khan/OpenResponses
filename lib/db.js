...
import type { Reaction } from "./components/thread";
...
// This isn't gonna hold up for big classes. Let's see how far we can get…
const fetchThreads = (
  flowID: string,
  classCode: string,
): Promise<{ [key: ThreadKey]: ThreadData }> =>
  getRef(flowID, classCode)
    .child("threads")
    .once("value")
    .then(snapshot => snapshot.val());

const watchRef = (ref, callback) => {
  const changeHandler = function(snap) {
    callback(snap.val());
  };
  ref.on("value", changeHandler);
  return () => ref.off("value", changeHandler);
};
...
const watchPartners = (
  ...
  classCode: string,
  userID: UserID,
  callback: ({ [key: string]: { userID: UserID } }) => any,
) => {
  return watchRef(
    getRef(flowID, classCode)
      .child("users")
...
const starClassCode = (flowID: string, classCode: string, starred: boolean) => {
  getRef(flowID, classCode).update({ starred });
};
