...
const referencePassage = (
  <div>
    ...
    <Paragraph>
      Longer passage that has many different causal factors
    </Paragraph>
  </div>
);
...
const modules = (getUserInput, getRemoteData) => [
  <TwoUpPrompt referenceComponent={referencePassage}>
    <Heading>
      1. What do you think was the main reason for X?
    </Heading>
  ...
  <TwoUpPrompt referenceComponent={referencePassage}>
    ...
    <Paragraph>
      {getUserInput(0).reason}
    </Paragraph>
    <Heading>
      1. What do you think is the most important factor in X?
    </Heading>
  ...
  <TwoUpPrompt referenceComponent={referencePassage}>
    <Heading>
      ...
      {" "}
      {factors[getUserInput(1).factor]}
      {" "}
      was the most important factor. Here's why:
    </Heading>
    <Paragraph>
      {getRemoteData("agreeingStudentResponse")}
    </Paragraph>
    <Heading>
      2. Did they cite the same evidence?
    </Heading>
  ...
  <TwoUpPrompt referenceComponent={referencePassage}>
    <Heading>
      ...
      {factors[getUserInput(1).factor]}
      {" "}
      was the most important factor. They believed that
      {" "}
      {factors[getRemoteData("disagreeingStudentResponse").choice]}
      {" "}
      was the most important factor instead. Here's why:
    </Heading>
    <Paragraph>
    ...
    <MultipleChoice dataKey="sameEvidence" choices={["Yes", "No"]} />
    {getUserInput(3).sameEvidence === undefined
      ? null /* Hide this part of the question until they choose an answer above. */
      : getUserInput(3).sameEvidence === 0
          ? <Heading>
  ...
  <TwoUpPrompt referenceComponent={referencePassage}>
    <Heading>
      1. Here is your original reasoning:
    </Heading>
    <Paragraph>
      {getUserInput(0).reason}
    </Paragraph>
    ...
    {getUserInput(4).choice === undefined
      ? null /* Hide this part of the question until they choose an answer above. */
      : getUserInput(4).choice === 0
          ? <Heading>
    ...
    fetcher: async ([factor], userID) => {
      if (factor === undefined) {
        ...
        const otherResponseSnapshot = await _getDatabase()
          .ref(`humanitiesC/${cohort}/inputs`)
          .orderByChild("1/factor")
          .equalTo(factor)
          .once("value");
