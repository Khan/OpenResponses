...
const modules = (getUserInput, getRemoteData) => [
  ...
  <TwoUpPrompt referenceComponent={humanitiesPassage}>
    ...
    <RichEditor dataKey="beliefSummary" />
    <Heading>
      Why did you choose that article as more believable?
    </Heading>
    <RichEditor dataKey="beliefJustification" />
  </TwoUpPrompt>,
...
const remoteDataRequirements = {
  otherStudentResponse: {
    ...
    fetcher: async ([interpretation], userID, cohort, { userState }) => {
      if (userState && userState.otherStudentResponseID) {
