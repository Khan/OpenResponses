...
const revieweeFetcher = ({
  ...
  if (
    userState.furthestPageLoaded === undefined ||
    userState.furthestPageLoaded < matchAtPageNumber ||
    inManagerInterface
  ) {
    return undefined;
  }
  ...
  let allValidUserIDs = Object.keys(
  ...
  // Put any pre-existing reviewee user IDs up front.
  allValidUserIDs = unrejectedReviewees
    .map(r => r.userID)
    .concat(
      allValidUserIDs.filter(
        u => !unrejectedReviewees.find(r => r.userID === u),
      ),
    );
