...
const flowName = "zoid_01";
const submissionPage = 5;
const waitingForReviewPage = 8;
...
const modules = (getUserInput, getRemoteData, dispatcher) => [
  ...
  getRemoteData("reviewees") && getRemoteData("reviewees").responses ? (
    <TwoUpPrompt
      ...
      passThroughInManagerUI
    >
      <Paragraph>Another student wrote this about the three methods:</Paragraph>
      <ResponseQuote
      ...
      />
      <Heading>
        Think about what they wrote. Help build on their ideas, and help answer
        their questions. Write at least two sentences.
      </Heading>
  ...
  extractFeedbackFromInbox(getRemoteData("_inbox")).length > 0 ? (
    ...
    >
      ...
      <RichEditor
        data={getUserInput(submissionPage).solnObservations} // index here matches 0-indexed index of card (see comments)
        quotedWork
        hideInReport
      />
      <Paragraph hideInReport>And you had this question:</Paragraph>
      <RichEditor
        data={getUserInput(submissionPage).solnQuestions}
        quotedWork
        hideInReport
      />
      <Paragraph>
...
const revieweeFetcher = async (
  ...
  const responseFromStudentData = studentData => {
    ...
    const moduleData = studentData.inputs.submitted
      ? studentData.inputs.submitted[submissionPage] // card number
      : studentData.inputs[submissionPage]; // card number
  ...
  if (userState.furthestPageLoaded < submissionPage + 1 || inManagerInterface) {
    return undefined;
    ...
    const allValidUserIDs = Object.keys(
      ...
      if (
        ...
        !otherResponse.inputs.submitted[submissionPage]
      ) {
        console.log(
...
export default {
  ...
  databaseVersion: 2,
  requiresEmail: true,
  reportSpec: [0, 1, 5, 6, 7, 8, 10, 11, 12],
  submissionThreshold: submissionPage,
  needsReviewModuleID: waitingForReviewPage,
};
