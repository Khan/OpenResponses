    ...
    fetcher: async (
      ...
      if (userState.furthestPageLoaded < 1) {
        ...
        allValidUserIDs.sort((a, b) => {
          console.log(a, b);
          const aAgrees = otherStudentResponses[a].inputs.submitted[
            0
          ].decision === decision;
          const bAgrees = otherStudentResponses[b].inputs.submitted[
            0
          ].decision === decision;
          if ((aAgrees && bAgrees) || (!aAgrees && !bAgrees)) {
            // Fallback users always come last.
            const aIsFallback = otherStudentResponses[
              a
            ].userState.isFallbackUser;
            const bIsFallback = otherStudentResponses[
              b
            ].userState.isFallbackUser;
...
export default { modules, remoteDataRequirements, databaseVersion: 2 };
